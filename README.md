# Hi, I'm Oswa 👋

🔐 BSc Cybersecurity Student @ Bahauddin Zakariya University, Multan  
🎯 Aspiring Bug Bounty Hunter | Security Researcher | Ethical Hacker  
🛠️ Building offensive security tools in C++ and Bash  
🌍 Goal: Financial independence through cybersecurity — remotely, in foreign currency  

## 🧰 Tools & Skills
- Languages: C++, Bash
- Platforms: Kali Linux, DVWA, HackTheBox, TryHackMe
- Tools: nmap, Burp Suite, sqlmap, gobuster, Hydra, Nikto, theHarvester
- Focus: OWASP Top 10, Web Application Security, Network Reconnaissance

## 🚀 Featured Projects
- 🔴 CommandInjector v2.0 — C++ command injection detection tool targeting OWASP A03:2021
- 🛡️ CyberShield v2.0 — Bash network risk assessment and reconnaissance tool

## 📈 Currently Learning
- Web Application Penetration Testing
- PortSwigger Web Security Academy labs
- Bug Bounty Methodology

## 📫 Connect
- LinkedIn: https://www.linkedin.com/in/oswa-fatima-cyber/
- TryHackMe: oswafatima06

